...
in phase 3, at step i:
- `result[i] = inv_all · prefix[i−1]` = (∏_{j≥i} a[j])⁻¹ · (∏_{j<i} a[j]) · (∏_{j<i} a[j])⁻¹ ... simplified: each `result[i] = a[i]⁻¹`
- `inv_all` is updated to `(∏_{j≥i−1} a[j])⁻¹` for the next iteration
...
a single inversion costs ~125 multiplications (see [[field]] § inversion). break-even vs. individual inversions:

```
N individual inversions: 125N multiplications
batch: 125 + 3(N−1) = 3N + 122 multiplications
```

batch wins for N ≥ 2. at N = 100: 422 vs. 12500 multiplications — a 30× speedup.
